...
void electron::compute_b2(double de, double degauss, double degthr){
    if (needL){
	    compute_b2(de, degauss, degthr, false, true);
	    if (this->rotate_spin_axes) compute_b2(de, degauss, degthr, true, true);
    }
    else{
...
void electron::compute_b2(double de, double degauss, double degthr, bool rotate_spin_axes, bool doL){
	if (ionode) printf("\n");
	if (ionode) printf("\n**************************************************\n");
	if (ionode) printf("electron: Analysis spin mixing, spin axis, spin-flip angle, etc.:\n");
	if (ionode) printf("          including orbital-filp as well\n");
	if (ionode && rotate_spin_axes){ printf("with rotated spin axes:\n"); sdir_rot.print(stdout, " %lg", true); }
	if (ionode) printf("**************************************************\n");
	if (ionode && !is_dir("electron_analysis")) system("mkdir electron_analysis");
	...
	if (exists("ldbd_data/ldbd_Umat.bin") && nb_eph == nb_dm){
		...
		double ***olf2ew = alloc_real_array(3, ne, nw), ***Nlfew = alloc_real_array(3, ne, nw),
			***olc2ew = alloc_real_array(3, ne, nw), ***Nlcew = alloc_real_array(3, ne, nw);
		double **olf2w_avg = alloc_real_array(3, nw), **Nlfw = alloc_real_array(3, nw),
			**olc2w_avg = alloc_real_array(3, nw), **Nlcw = alloc_real_array(3, nw);

		double eig_ik_ldeg[nb_dm], eig_jk_ldeg[nb_dm]; // eigenvalues of energy-degeneracy projections of spin matrices
		complex ovlp_l[nb_dm*nb_dm],Ui_ldeg[nb_dm*nb_dm], Uj_ldeg[nb_dm*nb_dm];
		...
		for (int id = 2; id >= 0; id--){
			for (int ik = mp->varstart; ik < mp->varend; ik++){
				// diagonalize spin matrices in energy-degenerate subspaces
				bool isI_ik = diagonalize_deg(s_rot[ik][id], e_dm[ik], nb_dm, degthr, eig_ik_sdeg, Ui_sdeg);
				bool isI_ik_l = diagonalize_deg(l_rot[ik][id], e_dm[ik], nb_dm, degthr, eig_ik_ldeg, Ui_ldeg);
				if (id == 2 && !isI_ik) count_nk_deg++;
				if (ionode && !isI_ik){
					printf("id= %d ik= %d\n", id, ik);
					printf_real_array(e_dm[ik], nb_dm, "e_dm: ");
					printf_complex_mat(s_rot[ik][id], nb_dm, "s: ");
					printf_real_array(eig_ik_sdeg, nb_dm, "eig_sdeg: ");
                    printf_real_array(eig_ik_ldeg, nb_dm, "eig_ldeg: ");
					printf_complex_mat(l_rot[ik][id], nb_dm, "l ");
					printf_complex_mat(Ui_sdeg, nb_dm, "U_sdeg: ");
				}
				...
				for (int jk = 0; jk < nk; jk++){
					...
					// diagonalize spin matrices in energy-degenerate subspaces
					bool isI_jk = diagonalize_deg(s_rot[jk][id], e_dm[jk], nb_dm, degthr, eig_jk_sdeg, Uj_sdeg);
					bool isI_jk_l = diagonalize_deg(l[jk][id], e_dm[jk], nb_dm, degthr, eig_jk_ldeg, Uj_ldeg);
                    ...
                    if (!isI_jk_l){
					    zgemm_interface(mtmp, ovlp_l, Uj_ldeg, nb_dm);
					    zgemm_interface(ovlp_l, Ui_ldeg, mtmp, nb_dm, c1, c0, CblasConjTrans);
                    }
					...
					for (int b1 = 0; b1 < nb_dm; b1++){
						...
						for (int b2 = 0; b2 < nb_dm; b2++){
							bool step_sf = eig_ik_sdeg[b1] * eig_jk_sdeg[b2] < 0; // true for a spin-flip transition
							bool step_lf = eig_ik_ldeg[b1] * eig_jk_ldeg[b2] < 0; // true for a orb-flip transition
							if (step_sf && abs(eig_ik_sdeg[b1] - eig_jk_sdeg[b2]) < min_ds[id])
								min_ds[id] = abs(eig_ik_sdeg[b1] - eig_jk_sdeg[b2]);

							if (step_lf && abs(eig_ik_ldeg[b1] - eig_jk_ldeg[b2]) < min_dl[id])
								min_dl[id] = abs(eig_ik_ldeg[b1] - eig_jk_ldeg[b2]);

							double o2_pair = ovlp[b1*nb_dm + b2].norm(); // scattering spin-mixing
						    double o2_pairl = ovlp_l[b1*nb_dm + b2].norm(); // scattering spin-mixing
							...
							for (int iw = 0; iw < nw; iw++){
								...
								if (step_lf){
									// frequency-dependent spin-flip overlap square and number of spin-flip transitions
									olf2ew[id][ie1[b1]][iw] += o2_pairl * weight_o2ew;
									Nlfew[id][ie1[b1]][iw] += weight_o2ew;
									olf2w_avg[id][iw] += o2_pairl * weight_o2w;
									Nsfw[id][iw] += weight_o2w;
								}
								else{
									// frequency-dependent spin-conserving overlap square and number of spin-conserving transitions
									olc2ew[id][ie1[b1]][iw] += o2_pairl * weight_o2ew;
									Nlcew[id][ie1[b1]][iw] += weight_o2ew;
									olc2w_avg[id][iw] += o2_pair * weight_o2w;
									Nlcw[id][iw] += weight_o2w;
								}
							}
						}
					}
			...
			mp->allreduce(b2_avg_2[id], MPI_SUM);
			mp->allreduce(min_ds[id], MPI_MIN);
			mp->allreduce(olf2ew[id], ne, nw, MPI_SUM); mp->allreduce(Nlfew[id], ne, nw, MPI_SUM);
			mp->allreduce(olc2ew[id], ne, nw, MPI_SUM); mp->allreduce(Nlcew[id], ne, nw, MPI_SUM);
		...
		mp->allreduce(count_nk_deg, MPI_SUM);
		mp->allreduce(max_diff_b2, MPI_MAX);
		mp->allreduce(olf2w_avg, 3, nw, MPI_SUM); mp->allreduce(Nlfw, 3, nw, MPI_SUM);
		mp->allreduce(olc2w_avg, 3, nw, MPI_SUM); mp->allreduce(Nlcw, 3, nw, MPI_SUM);
		// useful information
		if (ionode){
			printf("no. of k with energy degeneracy (tot no. of k): %lu (%d)\n", count_nk_deg, nk);
			printf("min. spin change along x: %lg\n", min_ds[0]);
		...
		for (int id = 2; id >= 0; id--){
			for (int ie = 0; ie < ne; ie++){
				...
				for (int iw = 0; iw < nw; iw++){
					...
					Nsfew[id][ie][iw] *= prefac_New;
					Nscew[id][ie][iw] *= prefac_New;
					olf2ew[id][ie][iw] = olf2ew[id][ie][iw] / Nlfew[id][ie][iw];
					olc2ew[id][ie][iw] = olc2ew[id][ie][iw] / Nlcew[id][ie][iw];
			...
			for (int iw = 0; iw < nw; iw++){
				...
				Nsfw[id][iw] *= prefac_Nw;
				Nscw[id][iw] *= prefac_Nw;
				olf2w_avg[id][iw] = olf2w_avg[id][iw] / Nlfw[id][iw];
				olc2w_avg[id][iw] = olc2w_avg[id][iw] / Nlcw[id][iw];
		...
		if (ionode && doL){
			string ldir[3]; ldir[0] = "x"; ldir[1] = "y"; ldir[2] = "z";
			for (int id = 2; id >= 0; id--){
				string fnamesf = "electron_analysis/olf2w_" + str_rot + ldir[id] + ".out",
					fnamesc = "electron_analysis/olc2w_" + str_rot + ldir[id] + ".out";
				FILE *fpsf = fopen(fnamesf.c_str(), "w"),
					*fpsc = fopen(fnamesc.c_str(), "w");
				fprintf(fpsf, "# transition energy (meV) olf2w Nlfw olf2*Nlf/Nf\n");
				fprintf(fpsc, "# transition energy (meV) olc2w Nlcw olf2*Nlf/Nf\n");
				for (int iw = 0; iw < nw; iw++){
					fprintf(fpsf, "%14.7le %14.7le %14.7le %14.7le\n", wgrid[iw] / eV * 1000, olf2w_avg[id][iw], Nlfw[id][iw], olf2w_avg[id][iw] * Nlfw[id][iw] / (sum_dfde / nk_full));
					fprintf(fpsc, "%14.7le %14.7le %14.7le %14.7le\n", wgrid[iw] / eV * 1000, olc2w_avg[id][iw], Nlcw[id][iw], olc2w_avg[id][iw] * Nlcw[id][iw] / (sum_dfde / nk_full));
				}
				fclose(fpsf); fclose(fpsc);

				for (int iw = 0; iw < nw; iw++){
					string fnamesf = "electron_analysis/olf2ew_" + str_rot + ldir[id] + "_w" + int2str(iw) + ".out",
						fnamesc = "electron_analysis/olc2ew_" + str_rot + ldir[id] + "_w" + int2str(iw) + ".out";
					FILE *fpsf = fopen(fnamesf.c_str(), "w"),
						*fpsc = fopen(fnamesc.c_str(), "w");
					fprintf(fpsf, "# elec. energy (eV) olf2ew Nlfew olf2e*Nlfe (for transition energy: %lg meV)\n", wgrid[iw] / eV * 1000);
					fprintf(fpsc, "# elec. energy (eV) olc2ew Nlcew olc2e*Nlce (for transition energy: %lg meV)\n", wgrid[iw] / eV * 1000);
					for (int ie = 0; ie < ne; ie++){
						fprintf(fpsf, "%14.7le %14.7le %14.7le %14.7le\n", egrid[ie] / eV, olf2ew[id][ie][iw], Nlfew[id][ie][iw], olf2ew[id][ie][iw] * Nlfew[id][ie][iw]);
						fprintf(fpsc, "%14.7le %14.7le %14.7le %14.7le\n", egrid[ie] / eV, olc2ew[id][ie][iw], Nlcew[id][ie][iw], olc2ew[id][ie][iw] * Nlcew[id][ie][iw]);
					}
					fclose(fpsf); fclose(fpsc);
				}
			}
		...
		dealloc_real_array(osf2ew); dealloc_real_array(Nsfew); dealloc_real_array(osf2w_avg); dealloc_real_array(Nsfw);
		dealloc_real_array(osc2ew); dealloc_real_array(Nscew); dealloc_real_array(osc2w_avg); dealloc_real_array(Nscw);
		dealloc_real_array(olf2ew); dealloc_real_array(Nlfew); dealloc_real_array(olf2w_avg); dealloc_real_array(Nlfw);
		dealloc_real_array(olc2ew); dealloc_real_array(Nlcew); dealloc_real_array(olc2w_avg); dealloc_real_array(Nlcw);
	}
	dealloc_real_array(b2kn);
